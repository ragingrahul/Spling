...
const Replies: NextPage<Props> = (props: Props) => {
    ...
    return (<div className="flex my-3 h-fit mx-[1.625rem] items-center">
        {props?.reply?.user?.avatar ? <img
            ...
            alt="avatar"
            className='rounded-full h-[60px] w-[60px] border-4 border-[#A0D8EF] hover:cursor-pointer' onClick={() => window.location.href=`./Dashboard/${props?.reply?.userId}`}
            /> :
        <img src='/ProfilePic.png' alt='ProfilePic' className='rounded-full h-[60px] w-[60px] border-4 border-[#A0D8EF] hover:cursor-pointer' onClick={() => window.location.href=`./Dashboard/${props?.reply?.userId}`}/>
        }
