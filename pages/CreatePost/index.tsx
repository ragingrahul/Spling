...
export default function CreatePost() {
      ...
      <div className="flex flex-col">
        <div className="mt-6">
          <input
            value={exercise}
            type="text"
            placeholder="Current Exercise"
            className="w-[370px]  p-1 bg-slate-200 border-2 border-[#A0D8EF] rounded-xl focus:outline-none text-[#565656] "
            onChange={(e) => {
              if (e.target) setExercise(e.target.value);
            }}
          />
        </div>
        <div className="mt-3">
          <input
            value={current}
            type="number"
            placeholder="Current Reps/Set"
            className="w-[370px] my-3 p-1 bg-slate-200 border-2 border-[#A0D8EF] rounded-xl focus:outline-none text-[#565656] "
            onChange={(e) => {
              if (e.target) setCurrent(e.target.value);
            }}
          />
        </div>
        <div className="mt-3">
          <input
            value={target}
            type="number"
            placeholder="Target Reps/Set"
            className="w-[370px] my-3 p-1 bg-slate-200 border-2 border-[#A0D8EF] rounded-xl focus:outline-none text-[#565656] "
            onChange={(e) => {
              if (e.target) setTarget(e.target.value);
            }}
          />
        </div>

        <button
          className="text-2xl font-semibold mt-[40px] bg-[#A0D8EF] w-[180px] self-center p-1 rounded-3xl"
          onClick={CreatePostProgression}
        >
          Add
        </button>
      ...
      <div className="flex flex-col">
        <div className="mt-3">
          <input
            value={exercise}
            type="text"
            placeholder="Completed Exercise"
            className="w-[370px] my-3 p-1 bg-slate-200 border-2 border-[#A0D8EF] rounded-xl focus:outline-none text-[#565656] "
            onChange={(e) => {
              if (e.target) setExercise(e.target.value);
            }}
        ...
        />
        <button
          className="text-2xl font-semibold mt-[20px] bg-[#A0D8EF] w-[180px] self-center p-1 rounded-3xl"
          onClick={CreatePostCompletion}
        >
          Add
        </button>
        <p className="self-center mt-[10px] font-medium">
  ...
  const CreatePostCompletion = async () => {
    ...
    if (image) {
      if (image.size > 1000000) return toast.warn("Upload image less than 1MB");
      if (!image.type.includes("image")) return toast.warn("Upload an image");
    } else {
              ...
              <div className="flex flex-row">
                <div
                  className={`p-[3px] px-[20px] ${
                    progression ? `bg-[#565656] text-slate-200` : `bg-[#A0D8EF]`
